    ...
    return (
      <div className={styles.header}>
        ...
        <span className={styles.auto} >
          ...
          <span className={styles.spanType} ><Icon type="folder" /> {categories[0].id ? categories.map(item => <Link to={`/categories/name/${item.name}`} key={`cat${item.name}`} >{item.name} </Link>) : "无"}</span>
